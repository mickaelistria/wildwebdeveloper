...
public class InitializeLaunchConfigurations {
  ...
  private static final String MACOS_DSCL_SHELL_PREFIX = "UserShell: ";
	...
	public static File getNodeJsLocation() {
		...
		if (nodeJsInfo != null) {
			...
			try (InputStream infoStream = nodeJsInfo.openStream()) {
				...
				File installationPath = unarchive(FileLocator.find(Activator.getDefault().getBundle(), new Path(properties.getProperty("archiveFile")));
				return new File(installationPath, properties.getProperty("nodePath"));
			} catch (IOException e) {
	...
	private static File unarchive(URL find) {
		try (InputStream stream = find.openStream()) {
			ArchiveInputStream archiveStream = new ArchiveStreamFactory().createArchiveInputStream(find.getPath(), stream);
			archiveStream.
		}
	}
